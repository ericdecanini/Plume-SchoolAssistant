...
public class TaskAdapter extends ArrayAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (context instanceof MainActivity) {
            // Get the preference for if the task due indicator should be used
            boolean useDueIndicator = PreferenceManager.getDefaultSharedPreferences(getContext())
                    .getBoolean(context.getString(R.string.KEY_THEME_TASKS_INDICATOR), true);

            // Get the due date of the task to determine if the indicator should be used
            ...
            // If the indicator should be used, tasks will be coloured yellow or red
            // Otherwise they will stay their default colour
            v.setBackground(context.getResources().getDrawable(R.drawable.touch_selector));
            if (useDueIndicator) {
                if (c1.get(Calendar.DAY_OF_MONTH) < c.get(Calendar.DAY_OF_MONTH) && c1.get(Calendar.MONTH) == c.get(Calendar.MONTH))
                    v.setBackground(context.getResources().getDrawable(R.drawable.touch_selector_red));
                else if (c1.get(Calendar.DAY_OF_MONTH) == c.get(Calendar.DAY_OF_MONTH) && c1.get(Calendar.MONTH) == c.get(Calendar.MONTH))
                    v.setBackground(context.getResources().getDrawable(R.drawable.touch_selector_yellow));
            }
        }
        ...
        // If this returns null, list_item_task2 is being used which only has icon and category
